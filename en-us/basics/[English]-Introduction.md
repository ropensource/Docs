...
**CBFT (Concurrent BFT)**. Currently the various commonly used xBFTs use synchronous processing, that is, the next block is produced after confirming a block. This method has a performance upper limit. PlatON adopts a concurrent BFT consensus, that is, block production and block validation are performed in concurrent, which greatly improves the rate of block production while ensuring BFT 1/3 fault tolerance.
