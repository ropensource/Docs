...
可以通过关注社区的公告及时获取升级提案的信息。当需要节点对升级提案进行投票时，社区会发布相关的[公告](https://forum.latticex.foundation/t/topic/1781 )。例如：
...
提案详情：
  ...
  本次链上升级代码地址：
  代码分支：https://github.com/PlatONnetwork/PlatON-Go/tree/release-0.11.0
...
根据从社区公告或区块链浏览器中获得的升级提案中的新版本号（假设为0.11.0），然后下载PlatON官方提供的脚本进行节点的升级。在**部署节点的机器**上进行如下步骤的操作：
...
- 执行命令，更新节点版本
  ...
  ```bash
  chmod u+x update_platon.sh && ./update_platon.sh 0.11.0 --xxxnet
  ```
  
  **注：**
  
  ​    0.11.0为指定需要升级的版本号（版本号可从社区发布的相关公告中或通过区块链浏览器从链上获取）。
  ...
  Begin to install platon version: 0.11.0==========
  Node paused successfully==========
  Do you want to continue? [Y/n] y
  Uninstall current platon version: 0.10.0 successfully==========
  Install platon version: platon0.11.0 successfully==========
